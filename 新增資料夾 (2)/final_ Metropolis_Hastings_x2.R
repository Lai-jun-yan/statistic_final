...
MH2 = function(sigma,mu1,mu2,y){
  
  a = 11
  b = (sum((y[y$gender==0,3]-mu1)^2)+sum((y[y$gender==1,3]-mu2)^2))/2
  
  sigma_star = rchisq(1,sigma)
  
  b_sigma = 10000000
  
  x = truncated_Cauchy(sigma_star,b_sigma)/truncated_Cauchy(sigma,b_sigma)
  
  y = runif(1)
  
  acceptance_probability = min(x,1)
...
for (i in 2:n) {
  ...
  sigma[i] = MH(sigma[i-1],mu_male[i-1],mu_female[i-1],birthweight)
...
hist(mu_male, breaks = seq(floor(min(mu_male)/1000)*1000, ceiling(max(mu_male)/1000)*1000, by = 100), xlim = c(2000, 4000), ylim = c(0, 0.005), main = "Density of µ1", xlab = "µ1", ylab = "Density",freq = FALSE)
...
hist(sigma2, breaks = seq(min(sigma2)-1.0e-06, max(sigma2)+1.0e-06, by = 1.0e-06), xlim = c(min(sigma2), max(sigma2)), ylim = c(0, 1.5e+05), main = "Density of 1/σ^2", xlab = "sigma2", ylab = "Density",freq = FALSE)
